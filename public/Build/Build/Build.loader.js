function createUnityInstance(t, n, c) {
    ...
    function r(e) {
        ...
        (n += "\n" + (r = r.startsWith(n) ? r.substring(n.length) : r).trim()) && l.stackTraceRegExp && l
            .stackTraceRegExp.test(n) && k(n, e.filename || t && (t.fileName || t.sourceURL) || "", e.lineno || t && (t.lineNumber || t.line) || 0)
    }
    ...
    var o, l = {
        ...
        webglContextAttributes: {
            preserveDrawingBuffer: !1, powerPreference: 2
        },
    ...
    l.SystemInfo = function () {
        ...
        for (var c = [["Windows (.*?)[;)]", "Windows"], ["Android ([0-9_.]+)", "Android"], ["iPhone OS ([0-9_.]+)", "iPhoneOS"], ["iPad.*? OS ([0-9_.]+)", "iPadOS"], ["FreeBSD( )", "FreeBSD"], ["OpenBSD( )", "OpenBSD"], ["Linux|X11()", "Linux"], ["Mac OS X ([0-9_\\.]+)", "MacOS"], ["bot|google|baidu|bing|msn|teoma|slurp|yandex", "Search Bot"]], d = 0; d < c.length; ++d) if (u = i(c[d][0], o, 1)) {
        ...
        var l = ((l = document.createElement("canvas")) && (gl = l.getContext("webgl2"), glVersion = gl ? 2 : 0, gl || (gl = l && l.getContext("webgl")) && (glVersion = 1), gl && (r = gl.getExtension("WEBGL_debug_renderer_info") && gl.getParameter(37446) || gl.getParameter(7937))), "undefined" != typeof SharedArrayBuffer),
        ...
        var c = void 0 !== a.headers.get("Content-Length")
        var d = function (e, t) {
            if (!t) { return 0 }
        ...
        return c || console.warn("[UnityCache] Response is served without Content-Length header. Please reconfigure server to include valid Content-Length for better download performance."), function o() {
            return void 0 === e ? a.arrayBuffer().then(function (e) {
                ...
                return i({
                    type: "progress", response: a, total: e.length, loaded: 0, lengthComputable: c, chunk: s ? t : null
                }), t
            }) : e.read().then(function (e) {
                if (e.done) {
                    if (h === d) return u;
                    if (h < d) return u.slice(0, h);
                    for (var t = new Uint8Array(h), n = (t.set(u, 0), f), r = 0; r < l.length; ++r) t
                        .set(l[r], n), n += l[r].length;
                    return t
                }
                return h + e.value.length <= u.length ? (u.set(e.value, h), f = h + e.value.length) : l
                    .push(e.value), h += e.value.length, i({
                    type: "progress",
                    response: a,
                    total: Math.max(d, h),
                    loaded: h,
            ...
            return i({
                type: "load", response: a, total: e.length, loaded: e.length, lengthComputable: c, chunk: null
            }), a.parsedBody = e, a
        })
        ...
        let a = fileToReadableStream(e).then(data => {
            return l.readBodyWithProgress(data, n, t.enableStreamingDownload)
        })
        // let a = fetch(e, t).then(data => {
        //     return l.readBodyWithProgress(data, n, t.enableStreamingDownload)
        // })
        return t && t.onProgress && (n = t.onProgress), a

    }, l.UnityCache = (f = {
        name: "UnityCache", version: 4
    }, p = {
        name: "RequestMetaDataStore", version: 1
        ...
        var o = this;
        return void 0 === b ? Promise.reject(new Error("Could not connect to cache: IndexedDB is not supported.")) : void 0 === window.caches ? Promise
            .reject(new Error("Could not connect to cache: Cache API is not supported.")) : new Promise(function (t, n) {
            try {
                function r() {
        ...
        e.objectStoreNames.contains(p.name) || (t = e.createObjectStore(p.name, {
            ...
            t.createIndex(e, e)
        })), e.objectStoreNames.contains(m) && e.deleteObjectStore(m), e.objectStoreNames.contains(g) && e
            .deleteObjectStore(g)
        ...
        return this.getMetaDataEntries().then(function (e) {
            ...
            return function e() {
                var t;
                return 0 === r.length ? Promise.resolve() : (t = r.pop(), i.cache.delete(t.url)
                    .then(function (e) {
        ...
        function d(n, r) {
            return fetch(n, r).then(function (e) {
                var t;
                return !c.enabled || c.revalidated ? e : 304 === e.status ? (c.revalidated = !0, i
                    .updateRequestMetaData(c.metaData).then(function () {
                    ...
                    return c.metaData.size = e.parsedBody.length, Promise.all([i
                        .storeRequest(n, t), i.updateRequestMetaData(c.metaData)]).then(function () {
        ...
        return a && (c.control = a.control, c.companyName = a.companyName, c.productName = a.productName, c.productVersion = a.productVersion), c.revalidated = !1, c.metaData = {
            url: s, accessedAt: Date.now(), version: c.productVersion
        }, c.response = null, c.enabled ? i.loadRequest(s).then(function (e) {
            var n, r, t;
            return e ? (n = e.response, r = e.metaData, c.response = n, c.metaData.size = r.size, c.metaData.updatedAt = r.updatedAt, "immutable" == c.control ? (c.revalidated = !0, i
                .updateRequestMetaData(r).then(function () {
                    x("'" + c.metaData.url + "' served from the indexedDB cache without revalidation")
                }), S(n, a.onProgress, a.enableStreamingDownload)) : (e = s, (t = window
                .location.href.match(/^[a-z]+:\/\/[^\/]+/)) && !e.lastIndexOf(t[0], 0) || !n
                .headers.get("Last-Modified") && !n.headers.get("ETag") ? (e = (a = a || {}).headers || {}, a.headers = e, n.headers.get("Last-Modified") ? (e["If-Modified-Since"] = n.headers.get("Last-Modified"), e["Cache-Control"] = "no-cache") : n.headers.get("ETag") && (e["If-None-Match"] = n.headers.get("ETag"), e["Cache-Control"] = "no-cache"), d(o, a)) : fetch(s, {
                method: "HEAD"
            }).then(function (t) {
                return c.revalidated = ["Last-Modified", "ETag"].every(function (e) {
                    return !n.headers.get(e) || n.headers.get(e) == t.headers
                        .get(e)
    ...
    function T(n) {
        D(n);
        let tmp_1 = l.companyName && l.productName
        let tmp_2 = l.cachedFetch
        let tmp_3 = l.fetchWithProgress
        var e = l.cacheControl(l[n])
        var t = tmp_3
        var r = l[n]
        var r = /file:\/\//.exec(r) ? "same-origin" : void 0;
        ...
        return t(tmp_l, tmp_t).then(function (e) {
            return i = e.parsedBody, s = l[n], new Promise(function (e, t) {
                for (var n in E) {
                    var r, o, a;
                    if (E[n].hasUnityMarker(i)) return s && console.log('You can reduce startup time if you configure your web server to add "Content-Encoding: ' + n + '" response header when serving "' + s + '" file.'), (r = E[n]).worker || (o = URL.createObjectURL(new Blob(["this.require = ", r
                        .require.toString(), "; this.decompress = ", r
                        .decompress.toString(), "; this.onmessage = ", function (e) {
                        e = {
                            id: e.data.id, decompressed: this.decompress(e.data.compressed)
                        };
                    ...
                    }, r.worker.callbacks = {}, r.worker.nextCallbackId = 0), a = r.worker.nextCallbackId++, r.worker.callbacks[a] = e, void r.worker
                        .postMessage({
                            id: a, compressed: i
                        }, [i.buffer])
                }
            ...
            let thatprotocol = location.protocol
            "file:" == thatprotocol ? d(t + ". Loading web pages via a file:// URL without a web server is not supported by this browser. Please use a local development web server to host Unity content, or use the Unity Build and Run option.", "error") : console.error(t)
        })
    }
