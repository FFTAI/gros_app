    ...
    <div class="container common-bkg">
      <div align="center" class="videoContainer">
        ...
        <div class="topScale">
          ...
          <div class="transverseRuler">
            <div
              class="tick"
              :class="{ longTick: (index % 6) - 5 === 0 }"
              v-for="(tick, index) in 47"
              :key="index"
              :style="{ left: index * 0.7292 + 'vw' }"
            ></div>
            <span
              class="positionVal"
              :style="{ left: 15.75 + pointX * 16 + 'vw' }"
              >{{ (pointX * 40).toFixed(1) }}°</span
            >
          </div>
        ...
        <div class="rightScale">
          ...
          <div class="verticalRuler">
            <div
              class="tick"
              :class="{ longTick: (index % 6) - 5 === 0 }"
              v-for="(tick, index) in 23"
              :key="index"
              :style="{ top: index * 0.7292 + 'vw' }"
            ></div>
            <span
              class="positionVal"
              :style="{ top: (14.7 + pointY * 15) / 2 + 'vw' }"
              >{{ (pointY * -17).toFixed(1) }}°</span
            >
          </div>
        ...
        <div class="indication">
          ...
          <img
            class="indicationImg"
            src="@/assets/images/image_indication.png"
            draggable="false"
          />
          <img
            class="shadowImg"
            src="@/assets/images/image_shadow.png"
            draggable="false"
          />
          <img
            class="pointer"
            src="@/assets/images/image_pointer.png"
            draggable="false"
          ...
          <div class="angleVal">
            <img
              class="turningImg"
              src="@/assets/images/icon_turningBody.png"
            />
            <span>{{ (direction * 45).toFixed(0) }}°</span>
          </div>
        </div>
      </div>
      <rtc-header :mode="mode"> </rtc-header>
      ...
      <div class="bottomBox flex-between fullWidth">
        <div class="leftBox flex-between">
          ...
          <div class="boxItem" style="justify-content: space-around">
        ...
        <div class="midBox flex-between">
          <div
            class="boxItem"
            :class="{
              chosedItem: currentstatus == 'Stand',
              opacity03: currentstatus == 'Start',
            }"
          ...
          <div
            class="boxItem"
            :class="{
              chosedItem: controlModel == 'inPlace',
              opacity03: currentstatus == 'Start',
            }"
          ...
          <div
            class="boxItem"
            :class="{
              chosedItem: controlModel == 'grasping',
              opacity03: currentstatus == 'Start',
            }"
      ...
      <transition name="fade">
        <div class="popDialog" v-show="currentstatus != 'Start'">
          <div
            v-if="controlModel == 'inPlace'"
            class="actionItem"
            :class="{ chosedAction: item.name == mode }"
            v-for="(item, index) in inPlaceList"
            :key="index"
            @click="choseMode(item.name)"
          >
            <img class="actionImg" :src="item.src" />
            <div>{{ $t(item.name) }}</div>
          </div>
          <div
            v-if="controlModel == 'grasping'"
            class="actionItem"
            :class="{ chosedAction: item.name == mode }"
            v-for="(item, index) in graspingList"
            :key="index"
            @click="choseMode(item.name)"
          >
            <img class="actionImg" :src="item.src" />
          ...
          <div
            v-if="controlModel == 'face'"
            class="actionItem"
            :class="{ chosedAction: item.name == mode }"
            v-for="(item, index) in faceList"
            :key="index"
            @click="showFace(item.val)"
          >
            <img class="actionImg" :src="item.src" />
            <div>{{ item.name }}</div>
          </div>
          <div
            v-if="controlModel == 'ai'"
            class="actionItem"
            :class="{ chosedAction: item.name == mode }"
            v-for="(item, index) in voiceList"
            :key="index"
            @click="getVoice(item.val)"
          >
            <img class="actionImg" :src="item.src" />
            <div>{{ item.name }}</div>
          </div>
...
export default {
  ...
  methods: {
    ...
    startPlay() {
      ...
      var url =
        "https://devfris.fftai.com/gr-rtc/rtc/v1/whep/?app=live&stream=" +
        this.robotName;
    ...
    mouseFunc(event) {
      ...
      if (this.rightMouseDown && this.ySpeed == 0) {
        ...
        this.operateWalk(this.direction * -45, this.velocity, this.ySpeed);
      }
    },
    onMousedown(event) {
      if (
        event.target &&
        (event.target.matches(".inImg") || event.target.matches("span"))
      ) {
        // 阻止事件的进一步传播
    ...
    onMouseup(event) {
      ...
      if (event.button == 0) {
        ...
        this.operateWalk(this.direction, this.velocity, this.ySpeed);
      }
    },
    //键盘操控
    handleKeyDown(event) {
      ...
      if (inPlaceInfo && this.controlModel == "inPlace")
        this.choseMode(inPlaceInfo.value);
      const faceKeys = {
      ...
      if (faceInfo && this.controlModel == "face")
        this.showFace(faceInfo.value);
      const graspingKeys = {
      ...
      if (graspingInfo && this.controlModel == "grasping")
        this.choseMode(graspingInfo.value);
    ...
    getVoice(e) {
      if (e == "action1") {
        this.choseMode("raiseHand");
      } else if (e == "action2") {
        this.choseMode("waveRHand");
      } else if (e == "action3") {
        this.choseMode("waveLHand");
      } else {
        let data = {
          command: "audio",
          data: {
    ...
    initMediaWs() {
      ...
      this.socket = new WebSocket(
        "wss://" + ip + "/terminal_audio/" + this.robotName
      );
      // 连接成功建立时触发
      this.socket.onopen = () => {
      ...
      navigator.mediaDevices
        ...
        .then((stream) => {
          ...
          const mediaStreamSource =
            that.audioContext.createMediaStreamSource(stream);

          const bufferSize = 0; // 根据需要选择缓冲区大小
          const scriptNode = that.audioContext.createScriptProcessor(
            bufferSize,
            1,
            1
          );
